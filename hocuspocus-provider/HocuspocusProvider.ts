import EventEmitter from "@/hocuspocus-provider/EventEmitter";
import {
  type CompleteHocuspocusProviderWebsocketConfiguration,
  HocuspocusProviderWebsocket,
} from "@/hocuspocus-provider/HocuspocusProviderWebsocket";
import { IncomingMessage } from "@/hocuspocus-provider/IncomingMessage";
import { MessageReceiver } from "@/hocuspocus-provider/MessageReceiver";
import { MessageSender } from "@/hocuspocus-provider/MessageSender";
import { AuthenticationMessage } from "@/hocuspocus-provider/OutgoingMessages/AuthenticationMessage";
import { AwarenessMessage } from "@/hocuspocus-provider/OutgoingMessages/AwarenessMessage";
import { CloseMessage } from "@/hocuspocus-provider/OutgoingMessages/CloseMessage";
import { QueryAwarenessMessage } from "@/hocuspocus-provider/OutgoingMessages/QueryAwarenessMessage";
import { StatelessMessage } from "@/hocuspocus-provider/OutgoingMessages/StatelessMessage";
import { SyncStepOneMessage } from "@/hocuspocus-provider/OutgoingMessages/SyncStepOneMessage";
import { SyncStepTwoMessage } from "@/hocuspocus-provider/OutgoingMessages/SyncStepTwoMessage";
import { UpdateMessage } from "@/hocuspocus-provider/OutgoingMessages/UpdateMessage";
import {
...
export class HocuspocusProvider extends EventEmitter {
  ...
  documentUpdateHandler(update: Uint8Array, origin: any) {
    ...
    this.send(
      UpdateMessage,
      { update, documentName: this.configuration.name },
      true,
    );
  }
  ...
  disconnectBroadcastChannel() {
    ...
    if (this.subscribedToBroadcastChannel) {
      bc.unsubscribe(
        ...
        this.boundBroadcastChannelSubscriber,
      );
  ...
  setAwarenessField(key: string, value: any) {
    if (!this.awareness) {
      throw new AwarenessError(
        `Cannot set awareness field "${key}" to ${JSON.stringify(
          value,
        )}. You have disabled Awareness for this provider by explicitly passing awareness: null in the provider configuration.`,
      );
    }
